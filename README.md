...
## 🌐 라이브 데모 (Live Demo)
프로그램 설치 없이 대시보드의 UI와 시뮬레이션 기능을 미리 체험해 볼 수 있습니다.
- **[대시보드 데모 보러가기](https://glory903-devsecops.github.io/endpoint-asset-classification-engine/)**

## 💻 로컬 실행 (Local Execution)
실제 로컬 컴퓨터의 파일을 실시간으로 분류하고 관리하려면 아래 명령어로 통합 환경을 실행하세요.
- **로컬 접속 주소**: [http://localhost:5173](http://localhost:5173) (백엔드 API 서버 연동)
...
-   **인플레이스(In-place) 동적 스캔**: 대시보드 상단에 경로를 입력하면 해당 폴더 내에서 실시간으로 정리가 수행됩니다.
